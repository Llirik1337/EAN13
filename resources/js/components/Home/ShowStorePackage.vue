    ...
    <div>
        <a-row type="flex" justify="center">
            <a-col class="package" :span="12">
                <a-input
                    ref="addDM"
                    placeholder="Input ean13"
                    v-model="code"
                    @pressEnter="endInputCode"
                ></a-input>
                <a-table :columns="columns" :data-source="data">
                    <a slot="name" slot-scope="text">{{ text }}</a>
                    <a slot="codedm" slot-scope="text">{{ text.slice(0,31) }}</a>
...
export default {
  ...
  methods: {
    ...mapActions(["addPackage", 'findPackage', 'updatePackage']),
    getViewBarcode() {
    ...
    async endInputCode() {
       const data = await this.findPackage(this.getCode())
        this.data = []
        if(data.result) {
            data.result.codedms.forEach(item => {
                this.addData(item.code.slice(0,31));
            })
            console.log(data);
            this.selectInput();
        }
